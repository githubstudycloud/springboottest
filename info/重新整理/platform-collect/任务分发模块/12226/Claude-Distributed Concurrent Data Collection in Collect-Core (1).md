...
## Prompt:
collector和processor的关系是什么，整个数据流向流程帮我梳理下

## Response:
让我帮你梳理Collector和Processor的关系以及整个数据流向流程。

### 1\. Collector与Processor的关系

```mermaid
stateDiagram-v2
    [*] --> Collector: 原始数据
    Collector --> ProcessorChain: 采集数据
    
    state ProcessorChain {
        [*] --> Processor1: 数据转换
        Processor1 --> Processor2: 数据验证
        Processor2 --> Processor3: 数据清洗
        Processor3 --> [*]: 数据格式化
    }
    
    ProcessorChain --> Repository: 处理后数据
    Repository --> [*]
```
...
@PostMapping("/collect")
public Response<String> collect(@RequestParam String code) {
    return enterpriseService.startCollect(code);
}
```
...
public void onTaskMessage(TaskMessage message) {
    ...
    // 2. 执行任务
    TaskResult result = handler.execute(createTaskContext(message));
}
```
...
protected List<Enterprise> doCollect(CollectContext<String> context) {
    ...
    // 2. 执行采集
    if (单条采集) {
        return collectSingle(code);
    } else {
        return collectBatch(shardIndex, shardTotal);
    }
}
```
...
public Enterprise save(Enterprise enterprise) {
    // 1. 保存到MongoDB
    Enterprise saved = mongoTemplate.save(enterprise);
    
    // 2. 更新缓存
    cacheManager.put(getCacheKey(saved.getCode()), saved);
...
1.  **Collector与Processor**:
    *   Collector完成采集后调用ProcessorChain
    *   ProcessorChain串联多个Processor顺序处理
    *   处理完成后返回结果给Collector
2.  **Service与Collector**:
    *   Service负责创建任务和调度
    *   Collector通过消息队列接收任务
    *   Collector执行完成后回调Service
3.  **Processor与Repository**:
    *   Processor完成处理后调用Repository
    *   Repository负责数据持久化
    *   Repository管理缓存一致性
...
## Response:
让我为您整理一个完整的测试流程和API调用示例：

1.  首先生成测试数据：

```bash
# 生成100条测试企业数据，编码从000001开始
POST http://localhost:8080/api/enterprise/generate
...
```bash
# taskId是上一步返回的实例ID
GET http://localhost:8080/api/enterprise/progress/{taskId}
```
...
```bash
# 分页查询
GET http://localhost:8080/api/enterprise/page?pageNum=1&pageSize=10&industry=制造业

# 单个查询
GET http://localhost:8080/api/enterprise/{code}

# 增量查询
GET http://localhost:8080/api/enterprise/increment?version=V20240326120000
```
...
# 4. 查询进度
curl http://localhost:8080/api/enterprise/progress/{taskId}
...
// 触发采集返回
{
    "code": "200",
    "message": "success",
    "data": "TASK_20240326120000_0001"  // 任务实例ID
}
...
1.  使用Postman：可以保存接口集合，方便重复测试
2.  使用curl：命令行快速测试
3.  使用swagger-ui：访问 [http://localhost:8080/swagger-ui.html](http://localhost:8080/swagger-ui.html)

注意事项：

1.  确保相关中间件(MongoDB、Redis、RabbitMQ)已启动
2.  检查application.yml中的配置是否正确
3.  观察应用日志输出，方便排查问题
4.  测试时注意保存返回的taskId，用于后续查询
