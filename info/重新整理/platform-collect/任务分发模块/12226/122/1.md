...
需要我详细解释某个部分吗？好的，我来为三个XML添加完整的resultMap映射：
...
主要修改点：
1. 为每个Mapper添加了resultMap配置
2. 将所有select语句的resultType替换为resultMap
3. 保证了数据库字段名(下划线)和Java属性名(驼峰)的正确映射
4. 处理了日期时间类型的映射

另外建议在MyBatis配置中添加驼峰命名转换配置：
...
这样配置后，MyBatis会自动处理下划线到驼峰的转换，但为了代码的可维护性和清晰性，还是建议保留完整的resultMap配置。对于MongoDB中处理版本数据，我们需要扩展VersionEntity的功能。我来展示完整的实现：

1. 首先修改VersionEntity，添加版本相关字段：
```java
@Data
@EqualsAndHashCode(callSuper = true)
public abstract class VersionEntity extends BaseEntity {
    protected String versionCode;    // 业务版本号，例如 V20241226115959_1
    protected LocalDateTime versionTime;  // 版本时间
    protected List<VersionHistory> versionHistory; // 版本历史记录

    @Data
    public static class VersionHistory {
        private String versionCode;
        private LocalDateTime versionTime;
        private Map<String, Object> changedFields; // 记录变更的字段
    ...
    public void upgradeVersion() {
        ...
        history.setVersionCode(this.versionCode);
        history.setVersionTime(this.versionTime);
...
3. 扩展EnterpriseRepository接口：
```java
public interface EnterpriseRepository extends MongoRepository<Enterprise, String> {
    // 根据编码和版本查询历史数据
    @Query("{'code': ?0, 'versionCode': {$lte: ?1}}")
    List<Enterprise> findHistoryByCode(String code, String versionCode);

    // 查询某个版本之后的所有变更数据
    @Query("{'versionCode': {$gt: ?0}}")
    List<Enterprise> findByVersionCodeGreaterThan(String versionCode);

    // 查询某个时间点之后的变更数据
    @Query("{'versionTime': {$gt: ?0}}")
    List<Enterprise> findByVersionTimeAfter(LocalDateTime versionTime);

    // 查询特定版本的数据
    @Query("{'code': ?0, 'versionCode': ?1}")
    Enterprise findByCodeAndVersion(String code, String versionCode);
}
```
...
@Service
@Slf4j
public class EnterpriseVersionService {
    private final EnterpriseRepository repository;

    public EnterpriseVersionService(EnterpriseRepository repository) {
        this.repository = repository;
    }

    // 保存新版本
    public Enterprise saveNewVersion(Enterprise enterprise, Map<String, Object> changedFields, String operator, String remark) {
        // 检查是否存在旧版本
        Enterprise oldVersion = repository.findByCode(enterprise.getCode());
        if (oldVersion != null) {
            // 复制历史版本记录
            enterprise.setVersionHistory(oldVersion.getVersionHistory());
            // 创建新的版本历史记录
            VersionHistory history = new VersionHistory();
            history.setVersionCode(oldVersion.getVersionCode());
            history.setVersionTime(oldVersion.getVersionTime());
            history.setChangedFields(changedFields);
            history.setOperator(operator);
            history.setRemark(remark);
            enterprise.getVersionHistory().add(history);
        }

        // 初始化或升级版本
        if (enterprise.getVersionCode() == null) {
            enterprise.initVersion();
        } else {
            enterprise.upgradeVersion();
        }
    ...
    // 查询历史版本
    public List<Enterprise> getHistoryVersions(String code) {
        Enterprise current = repository.findByCode(code);
        if (current == null) {
            return Collections.emptyList();
        }
        return repository.findHistoryByCode(code, current.getVersionCode());
    }

    // 查询特定版本
    public Enterprise getSpecificVersion(String code, String versionCode) {
        return repository.findByCodeAndVersion(code, versionCode);
    }

    // 获取版本差异
    public Map<String, Object> getVersionDiff(String code, String oldVersion, String newVersion) {
        Enterprise oldDoc = repository.findByCodeAndVersion(code, oldVersion);
        Enterprise newDoc = repository.findByCodeAndVersion(code, newVersion);
        
        if (oldDoc == null || newDoc == null) {
            return Collections.emptyMap();
        }

        return compareVersions(oldDoc, newDoc);
    }

    // 比较两个版本的差异
    private Map<String, Object> compareVersions(Enterprise oldVersion, Enterprise newVersion) {
        ...
        for (Field field : Enterprise.class.getDeclaredFields()) {
            ...
            try {
                ...
                if (!Objects.equals(oldValue, newValue)) {
                    diff.put(field.getName(), new Object[]{oldValue, newValue});
                }
            } catch (IllegalAccessException e) {
                log.error("比较版本差异失败", e);
            }
        }
...
public class EnterpriseService {
    private final EnterpriseVersionService versionService;

    // 更新企业信息
    public Enterprise updateEnterprise(Enterprise enterprise, String operator) {
        // 获取旧数据
        Enterprise oldData = repository.findByCode(enterprise.getCode());
        
        // 计算变更字段
        Map<String, Object> changedFields = new HashMap<>();
        if (!Objects.equals(oldData.getName(), enterprise.getName())) {
            changedFields.put("name", enterprise.getName());
        }
        // ... 其他字段比较
        
        // 保存新版本
        return versionService.saveNewVersion(enterprise, changedFields, operator, "更新企业信息");
    }

    // 获取历史版本
    public List<Enterprise> getHistoryVersions(String code) {
        return versionService.getHistoryVersions(code);
    }

    // 查询版本差异
    public Map<String, Object> getVersionDiff(String code, String oldVersion, String newVersion) {
        return versionService.getVersionDiff(code, oldVersion, newVersion);
    }
    
    // 查询增量数据
    public List<Enterprise> getIncrementalData(String lastVersion) {
        return repository.findByVersionCodeGreaterThan(lastVersion);
    }
}
```

6. 在MongoConfig中添加审计功能：
```java
@Configuration
@EnableMongoAuditing
...
```json
{
  ...
  "name": "企业A",
  "version": 2,
  "versionCode": "V20241226120000_2",
  "versionTime": ISODate("2024-12-26T12:00:00Z"),
  "versionHistory": [
    {
      "versionCode": "V20241226110000_1",
      "versionTime": ISODate("2024-12-26T11:00:00Z"),
      "changedFields": {
        "name": "企业A旧名称"
      },
      "operator": "admin",
  ...
  "createTime": ISODate("2024-12-26T11:00:00Z"),
  "updateTime": ISODate("2024-12-26T12:00:00Z"),
  "deleted": false
}
```